...
-- v_CUST_MIDDLE_NAME              CHAR(80)     
            nvl(BGIMNM,''),                           -- Dedope will have this data
--   v_CUST_LAST_NAME              CHAR(80)
           case when BGILNM is not null then BGILNM 
		   else cast('LAST_NAME'as nvarchar2(10))  end,                           -- Dedope will have this data
--   v_PREFERREDNAME             CHAR(50)
            gfcun,                   -- Done
-- v_SHORT_NAME                 CHAR(10)
            substr(gfdas,1,10),      -- Done
--   v_CUST_DOB                 CHAR(17)          
      case when BGIDOB < GFCOD and BGIDOB <> 0 and get_date_fm_btrv(BGIDOB) <> 'ERROR' then 
               case when sysdate <  to_date(get_date_fm_btrv(BGPF.BGIDOB),'YYYYMMDD') then '01-JAN-1970'
                    else to_char(to_date(get_date_fm_btrv(BGPF.BGIDOB),'YYYYMMDD'),'DD-MON-YYYY')
                 end
      else '01-JAN-1970'
     end,                   -- Completed
--   v_GENDER                   CHAR(10)
      case when BGIGEN is null then 'O'
         else to_char(bgpf.BGIGEN )
        end,                              -- Done -- if we insert as unspecified, it will be 11 characters, so simply U as unspecified
-- v_OCCUPATION_CODE              CHAR(5)
            convert_codes('OCCP',BGISAL),                                         -- mapping table required
--   v_NATIONALITY              CHAR(50)
            convert_codes('CNTRY',nvl(gfcnap,gfcnal)),                                -- nationality
--   v_NATIVELANG_TITLE            CHAR(5)
            '',                                         -- Default Blank    
--   v_NATIVELANG_NAME            CHAR(80)
            GFCUNA,                                         -- Default Blank 
-- v_DOCUMENT_RECIEVED            CHAR(1)
    'N',                                        -- check it -- Checked -- ok 
-- v_STAFF_FLAG                 CHAR(25)
            'N',                                        -- Check it  -- checked  -- ok  until mock 4
--   v_STAFFEMPLOYEEID             CHAR(50)
            '',                           -- MAPPING TABLE REQUIRED
--   v_MANAGER                 CHAR(100)
            convert_codes('RMCODE',GFACO),                          -- MAPPING TABLE REQUIRED
--   v_CUSTOMERNRE_FLAG            CHAR(1)
        case
         when  GFCNAL = 'SA' then 'N'
         else 'Y'
        end,                                            -- Check it
-- v_DATEOFBECOMINGNRE            CHAR(17)  
 case when  GFCNAL <> 'SA' then
                 case when GFCOD <> 0  and get_date_fm_btrv(GFCOD) <> 'ERROR'  then
                            to_char(to_date(to_char(get_date_fm_btrv(GFCOD)),'YYYYMMDD'),'DD-MON-YYYY')   -- Req Format : DD-Mon-YYYY, But now its DD-MM-YYYY
                      else ''
                 end
              ELSE '' 
            END,
--   v_CUSTOMER_MINOR            CHAR(1)
        check_minor(BGIDOB),                                              -- CHECK IT  -- Checked -- ok 
--   v_CUSTOMER_GUARDIAN_ID        CHAR(50)
            case when check_minor(BGIDOB) = 'Y' then 'GAURDIAN'
			else ''
			end,                                     -- DEFAULT BLANK
--   v_MINOR_GUARD_CODE            CHAR(1)
            case when check_minor(BGIDOB) = 'Y' then 'D'
			else ''
			end,                                     -- DEFAULT BLANK
...
--   v_REGION                 CHAR(50)
            '99999',                                      -- Zafin -- MAPPING TABLE REQUIRED
...
--   v_RATING_CODE            CHAR(5)
        get_segment(gfctp,gfc4r,4),                                      -- DEFAULT BLANK
...
--   v_CONSTITUTION_REF_CODE        CHAR(5)
            convert_codes('CONST',BGBCON),                                      -- DEFAULT BLANK
...
--   v_DESIGNATION            CHAR(50)
            convert_codes('DESIG',BGIOCD),                                      -- DEFAULT BLANK
...
-- v_MAIDEN_MOTHER_NAME            CHAR(50)
           BGPF.BGIMMN,                              -- CHECK IT    -- CHECKED -- OK 
...
-- v_Segmentation_Class            CHAR(100)
        get_segment(gfctp,gfc4r,2),                                   -- DEFAULT SET TO 'MRETL' need to workon this
--   v_Name                 CHAR(100)
            '',                                      -- DEFAULT BLANK
...
--   v_Introd_Status            CHAR(50)
            '',                                      -- DEFAULT BLANK
--   v_NativeLangCode            CHAR(10) --ENBARB not configured change later
            convert_codes('LANG',GFLNM),                               -- DEFAULT SET TO 'ENGLISH'
  --   v_MinorAttainMajorDate        CHAR(11)
                case when check_minor(BGIDOB) = 'Y' then 
                to_char(add_months(to_date(get_date_fm_btrv(bgpf.BGIDOB),'YYYYMMDD'),216),'DD-MON-YYYY')
             else   ''
              end,
...
-- v_SHORT_NAME_NATIVE            CHAR(50) This is reported by Infy as 10 characters but in upload sheet says 50
            substr(gfdasa,1,10),                                      -- DEFAULT BLANK
...
-- v_SUBSEGMENT                CHAR(50)
        get_segment(gfctp,gfc4r,3),                            -- DEFAULT BLANK
...
--   v_StrUserField8            CHAR(100)
    convert_codes_const('RETAIL_CONST_DETAILS',gfctp,
...
--   v_StrUserField13            CHAR(100)
        case 										-- DEFAULT BLANK	
			when GFCNAP in ('BH', 'KW','OM','QA','AE') then 'GCCNT'
			when GFCNAP = 'SA' then 'NATIONAL'
			else 'EXPAT'
		end,
...
--   v_StrUserField20            CHAR(100)
        get_segment(gfctp,gfc4r,1),                  -- DEFAULT BLANK
...
--   v_OCCUPATION_TYPE            CHAR(50)
            convert_codes('OCCP',BGIOCD),                                      -- MAPPING TABLE REQUIRED
--   v_BANK_ID                CHAR(8)
